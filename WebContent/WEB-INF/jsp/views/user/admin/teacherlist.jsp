...
<body>
					...
					<div class="col-lg-12">
						<div class="card">
							...
							<button type="button" class="btn btn-label btn-primary" data-toggle="modal" 
		           			...
		           			<button type="button" class="layui-btn" style="background-color: #33cabb" id="upexcel">上传教师信息</button>
	...
	layui.use('upload', function(){
	  ...
	  var uploadInst = upload.render({
	    elem: '#upexcel' //绑定元素
	    ,url: '/Graduation-Design-Management-System/admin/upload.action', accept:'file' //上传接口
	    ,done: function(res){
	      console.log("上传的数据为：" + res)
	    }
	    ,error: function(){
	      //请求异常回调
	    }
	...
	function jumpPage(pageNumber){
	  ...
	  function saveTeacherinfo() {
		  ...
		  if(phone!='' && !(/^1[34578]\d{9}$/.test(phone))){ //后面()中的new_phone改成phone,需要保持一致不然识别号码是否正确时会出错
		        alert("手机号码有误，请重填");  
		        return false; 
		    } 
		  if($("#power").val()=='-1') {
